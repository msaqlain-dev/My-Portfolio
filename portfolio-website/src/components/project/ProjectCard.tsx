...
import { Github, ExternalLink, Globe, Maximize2 } from 'lucide-react';
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{ boxShadow: 'inset 0 0 0 1px rgba(99,102,241,0.2), 0 0 40px rgba(99,102,241,0.07)' }}
        />
        ...
        {/* ── Mockup area — clickable ─────────────────────── */}
        <div
          className={`relative bg-gradient-to-b from-[#0a0a14] to-[#0d0d1a] cursor-pointer ${
            category === 'mobile' ? 'py-8 px-8 flex justify-center' : 'p-6'
          }`}
          onClick={() => setModalOpen(true)}
          role="button"
          tabIndex={0}
          aria-label={`View ${title} screenshots in full size`}
        ...
        >
          ...
          <div className="absolute inset-0 z-20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
            style={{ background: 'rgba(7,7,14,0.45)', backdropFilter: 'blur(1px)' }}
          >
            <motion.div
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/15 text-white text-xs font-semibold shadow-lg"
        ...
        <div className="flex flex-col flex-1 p-6">
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-[#1e1e35]">
            ...
            <motion.button
              onClick={() => setModalOpen(true)}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#111120] border border-[#1e1e35] text-slate-300 hover:text-white hover:border-indigo-500/40 text-sm font-medium transition-all duration-200"
